...
We implemented the Kalman filter using the provided MATLAB script to estimate the level of a constant fluid in a tank. The system parameters are defined as follows: Benzene density $\rho \approx 0.874$ g/cm$^3$, hydrostatic constant $c_p = 0.98\rho$, true level $s=340$ cm, and initial guess $\mu_s = 250$ cm with uncertainty $\sigma_s = 11$ cm.
...
\begin{figure}[H]
  \centering
  % Ensure the filename matches your uploaded file for the first execution
...
\begin{figure}[H]
  \centering
  % Ensure the filename matches your uploaded file for the second execution
...
\begin{itemize}
  ...
  \item \textbf{Unit Discrepancy in Visualization:} It is important to note that the observations (cyan stars) appear visually shifted downwards, centered around approximately $290$. This is not an error in the filter, but a visualization artifact. The plot displays the state in \textbf{cm} ($340$) and the measurements in \textbf{mbar} on the same Y-axis without scaling. Since $c_p \approx 0.856$ mbar/cm, the expected pressure measurement is $340 \times 0.856 \approx 291$ mbar. The filter internally handles this conversion via the observation constant $cst\_p$ ($H$).
