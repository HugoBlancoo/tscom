...
\begin{figure}[H]
    ...
    \begin{minipage}{0.49\textwidth}
        ...
        \label{fig:task5_zoom}
    \end{minipage}
\end{figure}

Figure~\ref{fig:task5_full} shows the magnitude response of the equivalent channel across a ±20 MHz span. The response exhibits the characteristic shape of a pulse-shaping filter combined with multipath effects, with maximum energy concentrated near the center frequency.

Figure~\ref{fig:task5_zoom} focuses on the OFDM passband (4 MHz bandwidth). The plot reveals that the channel response is \textbf{not frequency-flat} within the passband. Significant magnitude variations are visible across the band, indicating frequency-selective fading caused by the multipath propagation.
...
The received signal $z(t)$ is obtained by convolving the transmitted signal $x(t)$ with the multipath channel impulse response $\tilde{h}(t)$:
\[
